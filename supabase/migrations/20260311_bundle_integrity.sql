-- Migration: Bundle Integrity and Pending Task Lock
-- 1. Add cost_amount and is_bundle to user_tasks to track bundle costs for unfreezing
...
-- 2. Update complete_user_task to handle unfreezing and pending transitions
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(p.wallet_balance, 0.00), COALESCE(p.frozen_amount, 0.00), COALESCE(p.profit, 0.00), COALESCE(p.total_profit, 0.00),
    ...
    -- BALANCE SECURITY CHECK
    -- If wallet is negative, they must settle it (unless they are submitting the task that caused it)
    -- But we check this in the frontend too.
    IF v_wallet_balance < 0 THEN
        -- Allow submission only if it's the pending task
        SELECT id INTO v_pending_task_id FROM public.user_tasks 
        WHERE user_id = v_user_id AND task_item_id = p_task_item_id AND status = 'pending' LIMIT 1;
    ...
    IF v_pending_task_id IS NULL THEN
        ...
        IF v_pending_bundle IS NOT NULL AND (v_tasks_in_current_set + 1) = (v_pending_bundle->>'targetIndex')::INT THEN
            -- Trigger Bundle Modal on frontend is preferred, but this is a safety trigger
        ...
        ELSE
            ...
            v_cost_amount := 0; -- Regular tasks don't subtract cost in this flow except via bundle modal
        END IF;
    END IF;

    SELECT title INTO v_task_title FROM public.task_items WHERE id = p_task_item_id;

    -- UPDATE PROFILE: add back cost + bonus
    -- v_cost_amount + v_earned_amount is what gets unfrozen
    ...
    SET 
        ...
        frozen_amount = CASE WHEN v_is_bundle_task THEN GREATEST(0, frozen_amount - (v_cost_amount + v_earned_amount)) ELSE frozen_amount END,
